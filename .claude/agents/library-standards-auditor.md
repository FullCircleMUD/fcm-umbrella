...
`docs/`, `pyproject.toml`) plus the linter's output — but you do **not** read the library's *code* to
judge whether its principles are obeyed (that is the `doc-consistency-auditor`'s job). You **propose**; a
human decides and applies. You do not edit.
...
3. **Adjudicate divergences.** For each linter `warn`, read the library's `CLAUDE.md` and decide:
   - **Sanctioned divergence** — the gap is a *deliberate, documented, justified* divergence (e.g. a
     ...
     finding. The standard explicitly permits divergences stated in `CLAUDE.md`.
   ...
   - **Undocumented gap** — no documented divergence and not a placeholder case. **Flag it:** "fix it,
     ...
     legitimately doesn't need) from plain *quality gaps* (missing SPDX header, missing `__version__`,
     missing `progress.md`) — the latter you simply recommend fixing; nobody "declares a divergence" for
     a forgotten SPDX header.
4. **Apply the judgment checks the linter cannot:**
   - **`CLAUDE.md` structure.** Does it carry the standard's sections (what-this-is, status →
     ...
     library's principles section must include — *"the library does not own game concepts"* and *"no
     FCM-specific assumptions"*. Flag either if absent.
   ...
   - **Reduced-set doc spirit** — `docs/` is *content* only (how the library works), follows the umbrella
     conventions, and does not restate them. The linter already catches a stray
     `documentation-structure.md`; you judge the spirit (e.g. a docs file that re-documents the
     four-surface model instead of just the library).
